...
type StreamST struct {
	URL       string      `json:"url"`
	Broadcast BroadcastST `json:"broadcast"`
...
func (element *ConfigST) bcConfig(suuid string) (bool, string) {
	element.mutex.RLock()
	defer element.mutex.RUnlock()
	return element.Streams[suuid].Broadcast.Enable, element.Streams[suuid].Broadcast.URL
}
